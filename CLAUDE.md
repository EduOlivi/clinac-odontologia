...
- **Stack real: Next.js 16 (App Router, TypeScript) + Supabase + Resend, deployado no Cloudflare Workers via `@opennextjs/cloudflare`.** Não é mais um site estático — há build (`npm run build`), testes (`npm test`) e um backend de verdade (`app/api/`, `app/admin/`, RLS no Postgres).
